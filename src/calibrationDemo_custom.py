...
baseDir = '../custom-data-dump/' #data directory
objName = 'fruits/seq-1/' #object name (should correspond to a dir in data)
seqName = 'v1' #sequence name (subdirectory of object)
calName = 'calib-dec11' #calibration sourse (also a dir in data)
...
# firstFrame = os.path.join(baseDir, objName, seqName, '000001.jpg') #original
firstFrame = os.path.join(baseDir, objName, 'ffmpeg_776.jpg')
print("Perform horizontal extrinsic calibration")
tvec_h, rmat_h = computeExtrinsic(firstFrame, mtx, dist, dX, dY)
print('horizonatal calib results : ', tvec_h, rmat_h)
...
# np.savez(os.path.join(baseDir, objName, seqName, "extrinsic_calib.npz"), **ext_out) #original
np.savez(os.path.join(baseDir, objName, "extrinsic_calib.npz"), **ext_out)
