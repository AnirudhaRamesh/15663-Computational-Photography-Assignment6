...
#         img = cv2.resize(img, (1920, 1080)) # for our own capture
...
#     color_img = cv2.resize(color_img, (960, 540)) # for custom
    while True:
    ...
    img_points = np.vstack((x, y)).T.reshape(-1, 1, 2).astype('float')
    obj_points = np.array([[0, dX, dX, 0], [0, 0, dY, dY], [0, 0, 0, 0]]).T.reshape(-1, 1, 3).astype('float')
