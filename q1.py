...
I_hor_start_row, I_hor_end_row = 670, 768 #last is not inclusive
I_hor_start_col, I_hor_end_col = 200, 820 #last is not inclusive
I_hor_start_frame, I_hor_end_frame = 47, 129

I_ver_start_row, I_ver_end_row = 0, 300 #last is not inclusive
I_ver_start_col, I_ver_end_col = 250, 820 #last is not inclusive
I_ver_start_frame, I_ver_end_frame = 47, 129
...
extrinsics = np.load('data/frog/extrinsic_calib.npz')
...
rectangle_bounds_col = [315,750] #790]#815]
rectangle_bounds_row = [325,614]#630]
P_array = []
p_color_array = []
for row in range(rectangle_bounds_row[0],rectangle_bounds_row[1],4) : 
    for col in range(rectangle_bounds_col[0],rectangle_bounds_col[1],4) : 
        ...
        if st_offset_adjusted > 80 : st_offset_adjusted = 80
...
P_array_filtered = P_array_filtered[P_array_filtered[:,2]>1700]
...
# Display shadow edge pts
get_ipython().run_line_magic('matplotlib', '')
frame_no = 20
plt.figure()
plt.imshow(images[46+frame_no])
plt.scatter(pts_hor[frame_no][:,1],pts_hor[frame_no][:,0], c='r')
